...
const _setQrLoadingTip = (wait_time) => {
  ...
  if (wait_time < 1000 * 10) {
    $("#sidLabel").html(
      "<span>Generating your key pair...<span><br/><span>This process may take a while</span>"
    );
  } else {
    $("#sidLabel").html(
      '<span>Generating your key pair...<span><br/><span>Tired of waiting? <a href="?key_size=1024">Switch</a> to less secure (1024 bit) key pair</span>'
    );
  }
};
...
function init() {
  ...
  if (_query_string && _query_string.key_size) {
    try {
      ...
      console.warn(
        "failed to parse key size:",
        _query_string.key_size,
        "error:",
        e
      );
    }
  ...
  if (
    _query_string &&
    (_query_string.onlyinfo === true || _query_string.onlyinfo === "true")
  ) {
    $("#qrplaceholder").hide();
  } else {
    $("#qrcode_loading").show();
...
function requestInit() {
  ...
  $.post(
    "init.php",
    { PUBLIC_KEY: toSafeBase64(PEMtoBase64(_crypt.getPublicKey())) },
    "json"
  )
    .done(function (data) {
      if (data.status === true) {
    ...
    _crypt.getKey(() => {
      ...
      if (supportLocalStorage()) {
        ...
        localStorage.setItem(
          LOCAL_STORAGE_PRIVATE_NAME,
          _crypt.getPrivateKey()
        );
      } else {
...
function initAsyncAjaxRequestSSE() {
  credentialsEventSource = new EventSource(
    `getcredforsid.php?sid=${_sid}&token=${_token}`
  );
...
function onFail(data, textStatus, jqXhr) {
  errorMsg =
    data != undefined && data.status === false
      ? "Error: " + data.message
